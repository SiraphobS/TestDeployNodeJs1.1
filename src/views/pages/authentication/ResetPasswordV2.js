...
const ResetPasswordV2 = props => {
  ...
  const verifyToken = async () => {
    ...
    await axios.post(`${process.env.REACT_APP_API_ENDPOINT}/VerifyToken`, data)
  ...
  const handleSubmit = async (event, errors) => {
    if (errors && !errors.length) {
      ...
      if (passwordNew === passwordConfirm) {
        ...
        await axios.post(`${process.env.REACT_APP_API_ENDPOINT}/ResetForgotPassword`, dataChange)
          .then(async function (response) {
            console.log(response)
        ...
        notify('danger', 'ERROR', 'New Password and Confirm Password not Match')
      }
          ...
          <Card className='mb-0'>
            <CardBody>
              <Link className='brand-logo' to='/' onClick={e => e.preventDefault()}>
                ...
                <h2 className='brand-text text-primary ml-1'>VivaPerfect</h2>
              </Link>
              ...
              <AvForm className='auth-forgot-password-form mt-2' onSubmit={handleSubmit}>
                ...
                <Button.Ripple color='primary' block disabled={!passwordNew.length || !passwordConfirm.length || !username.length}>
                  Set New Password
              </Button.Ripple>
              </AvForm>
              {/* </Form> */}
